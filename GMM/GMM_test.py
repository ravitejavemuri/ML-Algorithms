# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 15:31:25 2019
...
class GMM:
    ...
    def fit(self, features):
        ...
        #print('features', features[271])
        #gmm_model = GaussianMixture(n_components=4)
        # gmm_model.fit(features)
        #gmm_log = gmm_model.score(features, y=None)
        #gmm_log_p = gmm_model.score_samples(features)
        while True:
            ...
            for i in np.arange(n):
                ...
                for j in np.arange(self.n_components):
                    ...
                    #print('normal densities', norm_densities)
            # Estimate log likelihood
            X = np.array([np.dot(self.mixing_probs.T, norm_densities[i]) for i in np.arange(n)])
            log_vector = np.absolute(np.log(X))
            ...
            return  [log_vector, self.mixing_probs, norm_densities, self.means, self.covariances]
    ...
    def score(self,features,mean, covar ):

        n, _ = features.shape
        norm_densities = np.empty((n, self.n_components), np.float)
        #print('features_shape',mean, covar)
        for i in np.arange(n):
            x = features[i]
            for j in np.arange(self.n_components):
                norm_densities[i][j]= self.multivariate_normal_pdf(x, mean[j], covar[j])
                #print('normal densities', norm_densities)
        # Estimate log likelihood
        X = np.array([np.dot(self.mixing_probs.T, norm_densities[i]) for i in np.arange(n)])
        #print("x is",X)
    ...
    def multivariate_normal_pdf(self, x, mean, covariance):
        ...
        vector x.
        """
        #print("from gmm",mean, covariance)
        ...
        result = np.exp(-0.5 * exponent) / np.sqrt(np.absolute(cov_det * np.power(2 * np.pi, self.n_components)))
        return result
    ...
    def _initialise_parameters(self, features):
        ...
        if not self.means or not self.covariances:
           ...
           # print('features, features_suffeled', features_shuffled)
